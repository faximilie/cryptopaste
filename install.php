...
<?php
	...
	$printMap = array(
		...
		install.php -h
		install.php -V
		install.php [-v <verbosity>] --db:host=<host> --db:port=<port> --db:user=<user> --db:pass=<pass>\n",
	...
	function writeConfiguration($templateConfigFileLocation, $realConfigFileLocation, $options){
		...
		//TODO: clean options of quotes and backslashes to avoid escaping inside the config file
		...
		// TODO Update preg_replace to work with new config
		...
		foreach($options as $option=>$value){
			...
			$configContents = preg_replace('/(define\("*.", ")('.$option.')("\);)/i',
